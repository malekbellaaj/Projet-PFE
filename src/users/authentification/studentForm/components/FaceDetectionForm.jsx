// frontend/src/users/authentification/studentForm/components/FaceDetectionForm.jsx
...
export default function FaceDetectionForm({ onFaceCapture }) {
    ...
    const interval = setInterval(async () => {
      if (videoRef.current) {
        const detections = await faceapi
          ...
          .withFaceLandmarks()
          .withFaceExpressions()
          .withFaceDescriptors(); // Ajout des descripteurs faciaux
  ...
  const handleCapture = ({ imageData, descriptor }) => {
    setHasCaptured(!!imageData && !!descriptor);
    onFaceCapture({ imageData, descriptor }); // Transmettre l'image et le descripteur à MainForm
    console.log("Image et descripteur capturés :", { imageData, descriptor });
  };
        ...
        <>
          ...
          {!hasCaptured && (
            <Alert severity="warning" sx={{ mt: 2 }}>
              Veuillez capturer une image et un descripteur facial pour continuer.
            </Alert>
          )}
        </>
      ) : (
