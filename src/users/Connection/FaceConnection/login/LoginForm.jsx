// frontend/src/users/Connection/FaceConnection/login/LoginForm.jsx
...
const onSubmit = async (data) => {
  try {
    ...
    if (role === "admin") {
      ...
      const studentSituation = response.data.user.studentSituation;
      ...
      if (studentSituation === "normal") {
        redirectPath = "/dashboard-student";
      } else if (studentSituation === "hyperactif") {
        redirectPath = "/dashboard-student-hyperactif";
      } else {
        throw new Error(`Situation de l'élève non prise en charge: ${studentSituation}`);
      }
      userRole = "student";
    }
  ...
  return (
    <Container maxWidth="sm">
      ...
      <Box
        component="form"
        action={role === "admin" ? "/api/login-admin" : "/api/students/login"}
        method="POST"
          ...
          <Grid sx={{ width: "100%" }}>
            <Stack direction="row" justifyContent="space-between" mb={2}>
              ...
              <Typography
                ...
                role="link"
                tabIndex={0}
                onKeyDown={(e) => e.key === "Enter" && navigate.push("/reset-password")}
              >
                Mot de passe oublié ?
              </Typography>
            </Stack>
          ...
          <Grid sx={{ width: "100%" }}>
            ...
            >
              ...
              <span
                ...
                role="link"
                tabIndex={0}
                onKeyDown={(e) => e.key === "Enter" && navigate.push("/register")}
              >
                Créer un compte
              </span>
            </Typography>
...
//     if (response.status === 200) {
//       // ✅ ICI ON STOCKE LE TOKEN ET LE RÔLE
//       localStorage.setItem("token", response.data.token);
//       localStorage.setItem("user", JSON.stringify(response.data.user));
//       localStorage.setItem("role", userRole);
