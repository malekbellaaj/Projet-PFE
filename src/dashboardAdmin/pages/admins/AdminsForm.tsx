...
const AdminsForm = ({ onAddAdmin }: AdminsFormProps) => {
  ...
  const handleFormSubmit = async () => {
    ...
    if (!formData.password) {
      ...
      errors.password =
        "Le mot de passe doit contenir au moins 8 caractères, une lettre, un chiffre et un symbole";
      hasError = true;
    }
    ...
    try {
      ...
      const newAdmin = {
        id: response.data.admin?.id || Date.now().toString(),
      ...
      MySwal.fire({
        title: "Erreur",
        text: error.response?.data?.message || "Impossible d'ajouter l'administrateur.",
        icon: "error",
        confirmButtonColor: "#d33",
      });
    }
...
//       const newAdmin = {
//         id: response.data.admin?.id || Date.now().toString(), // Fallback en cas d'absence d'ID
//         fullName: formData.fullName,
//         email: formData.email,
//         createdAt: new Date().toISOString(),
//       };
